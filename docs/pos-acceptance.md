...
| **A collection still can't be paid with store credit**              | `store_credit` remains off `COUNTER_TENDER_METHODS`: no credit spend is wired into `markCollected`, so accepting it would mark an order paid against a balance nothing deducted. The last difference between the two allowlists                                                                                        |
