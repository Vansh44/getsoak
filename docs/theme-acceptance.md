...
| Vitrine | Passes the automated package gate as published preset `vitrine@0.1.0` on `storefront-classic@1`; asset provenance is recorded  | Production demo seeded and route-checked on 2026-08-22 (shop, PDP, content pages, all ten assets). Catalog and signup exposure are live.          | **Not recorded against this gate**                                                                                                            | Reviewed and approved by the store owner — a single reviewer. §5 asks for two, at least one non-author | Published; acceptance open |
...
met. Studio, Ritual and Vitrine are public, but publication does not erase their
open performance and human-review gates; they must not be described as fully
approved until those rows close. Vitrine was published on the store owner's
review alone and has no accessibility or Lighthouse run against it — recorded
here so the gap is visible rather than assumed closed by the fact that it
shipped. Their detailed production record is
`docs/theme-release-audit-2026-08-11.md`.
