...
| Analytics **remaining phases** | Phase 1, Phase 2a–2d and Search Console Phase 3a–3c shipped through 2026-08-20. Migration `20260819_0006_search_metrics` is applied; ingest and seven delayed-data dashboard cards are complete. Domain settings now exposes Google ownership, sitemap, last-attempt and error state with a permission-gated retry, and the Failures feed includes tenant-safe merchant plus explicit cross-store operator views. Cloud Scheduler still needs deployment-time creation/verification. Next build: A8 cleanup—delete obsolete custom-domain Google properties and honor token `expires_in`—before drill-down/CSV and first-party traffic. Spec: `docs/analytics-and-search-console-plan.md` |
