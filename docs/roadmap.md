...
| Analytics **remaining phases**  | Phase 1 and Phase 2a–2c shipped through 2026-08-19: range/metric contract, per-admin versioned layouts, named sections/card sizes, scope-safe location selection, AOV, units sold, top products, sales by channel, and sales by location. Ledger migration `20260818_0005_analytics_dashboard_layouts` is applied; Phase 2b/2c require no further SQL. Next: payment-method/new-returning/discount/refund/inventory widgets → Search Console → first-party traffic. Spec: `docs/analytics-and-search-console-plan.md` |
