...
| Analytics **remaining phases**  | Phase 1 and Phase 2a–2d shipped through 2026-08-19: range/metric contract, per-admin versioned layouts, named sections/card sizes, scope-safe location selection, and the complete migration-free commerce set (AOV, units/products, channel/location/payment, customer mix, discounts, returns/refunds, inventory velocity). Ledger migration `20260818_0005_analytics_dashboard_layouts` is applied; Phase 2b–2d require no further SQL. Next: Search Console ingestion/cards → first-party traffic. Spec: `docs/analytics-and-search-console-plan.md` |
