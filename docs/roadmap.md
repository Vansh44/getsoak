...
| Analytics **remaining phases** | Phase 1, Phase 2a–2d and Search Console Phase 3a–3b shipped through 2026-08-19. Migration `20260819_0006_search_metrics` is applied; the ingest has tenant-safe source epochs, metric buckets, a fleet rate limiter and resumable `/api/cron/search-metrics`. The dashboard now has seven delayed-data Google Search cards with source/freshness disclosure and distinct unlaunched, collecting, zero-visibility, ready and error behavior. Cloud Scheduler still needs deployment-time creation/verification. Next build: merchant and operator indexing-health surfaces, then A8 cleanup before first-party traffic. Spec: `docs/analytics-and-search-console-plan.md` |
