...
| Analytics **remaining phases**  | Phase 1 and Phase 2a/2b shipped 2026-08-18: range/metric contract, per-admin server layout, stale-tab protection, reset-to-default row deletion, dormant permission entries, one-time localStorage import, named sections, cross-section movement, and semantic card sizes. Ledger migration `20260818_0005_analytics_dashboard_layouts` is applied; existing version 1 rows upgrade through the JSON read/write boundary without new SQL. Next: commerce widgets/location selector → Search Console → first-party traffic. Spec: `docs/analytics-and-search-console-plan.md` |
