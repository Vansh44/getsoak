...
| Analytics **remaining phases** | Phase 1, Phase 2a–2d and Search Console Phase 3a–3d shipped through 2026-08-20. Migration `20260819_0006_search_metrics` is applied; ingest and seven delayed-data dashboard cards are complete. Domain settings exposes Google ownership, sitemap, last-attempt and error state with a permission-gated retry, and the Failures feed includes tenant-safe merchant plus explicit cross-store operator views. Detached custom domains now release their Google property and ownership record, and access-token caching follows Google's real expiry. Cloud Scheduler still needs deployment-time creation/verification. Next build: analytics drill-down and CSV export, then pixels/consent or first-party traffic only after the privacy decision. Spec: `docs/analytics-and-search-console-plan.md` |
