...
| Analytics **remaining phases** | Phase 1 and Phase 2a–2d shipped through 2026-08-19. Search Console Phase 3a now has tenant-safe source epochs, metric buckets, a fleet rate limiter and resumable `/api/cron/search-metrics`; ledger migration `20260819_0006_search_metrics` is pending application, and the Cloud Scheduler job must be created after deploy. Next: apply/verify that migration, then build the delayed-data Search cards and indexing health states before first-party traffic. Spec: `docs/analytics-and-search-console-plan.md` |
