...
| Analytics **phases 2b–4**       | Phase 1 and Phase 2a persistence shipped 2026-08-18: per-admin server layout, stale-tab protection, reset-to-default row deletion, dormant permission entries, and one-time localStorage import. Ledger migration `20260818_0005_analytics_dashboard_layouts` is applied. Next: named sections/card sizes → commerce widgets/location selector → Search Console. Spec: `docs/analytics-and-search-console-plan.md` |
