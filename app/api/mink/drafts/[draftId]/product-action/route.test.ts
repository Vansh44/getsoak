...
vi.mock("@/lib/mink/product-actions", () => ({
  previewMinkProductAction: holder.preview,
  executeMinkProductAction: holder.execute,
  previewMinkProductActionRollback: holder.rollback,
}));
...
describe("Mink Phase 4A product action API", () => {
  it("rejects cross-origin mutation requests before authentication", async () => {
  ...
  it("refuses browser-supplied replacement text", async () => {
    const response = await POST(
      request({
        action: "execute",
        approvalId: APPROVAL_ID,
        description: "Unreviewed browser value",
      }),
      PARAMS,
    );
  ...
  it("executes an approval inside the draft URL scope and invalidates product caches", async () => {
    const response = await POST(
    ...
    expect(holder.revalidatePath).toHaveBeenCalledWith("/shop/tea");
...
function request(body: unknown, origin = "https://acme.storemink.com") {
  return new Request(
    `https://acme.storemink.com/api/mink/drafts/${DRAFT_ID}/product-action`,
    {
      method: "POST",
      headers: {
