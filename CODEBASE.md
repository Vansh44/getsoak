...
> **Mink Phase 7C map update (2026-09-04):** the Phase 7A-only wording in the
> compact AI row above is superseded by Phases 7B–7C. With drafting plus Builder
...
> save only the reviewed section to `store_pages.sections`. The transaction
> rechecks tenant/admin/permission/entitlement/tool/proposal/page/section/hash,
> is idempotent, and writes an append-only outcome. It still has no new-section,
> header/footer, publish, rollback, repository, shell or deployment authority.
> Explicit authorised code-generation intent uses HIGH thinking; human review
> and execution use no model call and no second credit charge.
...
│       │                      # exact five-minute Builder-draft save approval, no publication.
...
│   │                          # draft-only section save, idempotency and audit. thinking.ts selects HIGH
...
expiry produces one audit outcome. The write never touches `published_sections`,
status or `published_at`; section creation, header/footer changes, publication,
rollback, repository, shell and deployment remain unavailable. Sales and orders reuse the dashboard's
...
the trusted `store_id`, and direct customer PII is minimized/masked.
     ...
     publishes the guide; migrations 0036–0058 and 0072–0078 keep
