...
| AI        | Gemini (`lib/ai/gemini.ts`); per-store brand voice (`lib/ai/brand-voice.ts` + `store_brand_profiles`) with plan-capped usage metering (`lib/ai/quota.ts`); task prompts in `brand/tasks/`. The dashboard Mink drawer has a disabled-by-default, invited-store Vertex/Gemini 3.7 beta in `lib/mink/` + `app/api/mink/`: Phase 2 streams permission/location-aware store, catalogue, sales, order, inventory and Help reads; Phase 3 adds a separate operator opt-in for private product-description, product-SEO, blog, coupon-email and reusable customer-message proposals. Draft tools apply the store brand voice, show expected weighted credits, charge atomically, and save immutable private versions with rollback; they cannot publish, send, contact a customer or mutate a live business row. Operators control both gates and inspect redacted cross-store run/draft metrics at `/dashboard/mink`; the 50-case live harness is `npm run mink:eval`. When disabled or uninvited, Mink retains the canned coming-soon response. Architecture and phased rollout are tracked in `docs/mink-ai-dashboard-plan.md`. |
...
│   │   │                      # versions and restores proposals without a publish/send path.
...
│   │                          # admin-private immutable versions and rollback. No tool can
│   │                          # publish, send, contact customers or change a business row.
...
20a. **Mink AI dashboard agent — invited read beta plus private drafting beta.** This is separate
...
the trusted `store_id`, and direct customer PII is minimized/masked.
     ...
     and the updated Help contract. Guarded live mutations, approvals and
     ...
     publishes the guide; migrations 0036–0040 keep
