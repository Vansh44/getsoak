...
| AI        | Gemini (`lib/ai/gemini.ts`); per-store brand voice (`lib/ai/brand-voice.ts` + `store_brand_profiles`) with plan-capped usage metering (`lib/ai/quota.ts`); task prompts in `brand/tasks/`. The dashboard Mink drawer has a disabled-by-default, invited-store Vertex/Gemini 3.7 beta in `lib/mink/` + `app/api/mink/`: Phase 2 streams permission/location-aware store, catalogue, sales, order, inventory and Help reads; Phase 3 adds a separate operator opt-in for private product-description, product-SEO, blog, coupon-email and reusable customer-message proposals. Phase 4A adds independently gated, human-approved product-description and product-SEO actions: exact ten-minute previews are bound to the saved draft and product content version, execution is field-limited/idempotent/transactional, conflicts fail closed, outcomes are append-only audited, and rollback requires an unchanged post-action checkpoint. The model receives no general product write tool; price, stock, variants, status, publication, images, other domains and customer contact remain unavailable. Operators control every gate and inspect redacted cross-store run/draft metrics at `/dashboard/mink`; the 50-case live harness is `npm run mink:eval`. When disabled or uninvited, Mink retains the canned coming-soon response. Architecture and phased rollout are tracked in `docs/mink-ai-dashboard-plan.md`. |
...
│   │   │                      # versions/restores proposals and provides Phase 4A's exact,
│   │   │                      # explicitly approved product-text apply/rollback UX.
...
│   │                          # fail-closed global + invited-store access; trusted actor,
...
│   │                          # admin-private immutable versions and rollback. Phase 4A adds
│   │                          # product-action-types.ts + product-actions.ts: exact short-lived
│   │                          # approvals, per-tool gates, field-limited transactional writes,
│   │                          # idempotency/conflict checks, immutable outcomes and safe rollback.
│   │                          # No model tool can publish, send or perform a general record write.
...
│                              # 0042 adds Phase 4A product-action gates, exact approvals, immutable
│                              # action audit and the guarded-action Help Centre contract.
├── scripts/
...
20a. **Mink AI dashboard agent — invited read, private drafting and guarded product-action beta.** This is separate
from the public Help Centre assistant. `MINK_AI_ENABLED` is a private,
server-read kill switch, while `MINK_BETA_REQUIRE_INVITE` defaults true and
requires an enabled `mink_store_access` row: either failure keeps the original
canned drawer. An operator can add/remove the invitation from the store detail
...
timeout. Its read tools cover store profile, catalogue summary, product search,
...
the trusted `store_id`, and direct customer PII is minimized/masked.
     ...
     build-file kill switch defaults false and the invitation requirement
     defaults true. Enabling `_MINK_AI_ENABLED=true` therefore exposes the real
     agent only to operator-invited stores; a bare build plus the staging and
     production triggers remain globally fail closed.
     ...
     draft text or approval payloads. Product price/inventory/variants/status/publication/
     images, product creation, other domains, customer contact and StoreMink
     source/deployment access remain absent. Migration
     ...
     publishes the guide; migrations 0036–0042 keep
...
- **Gemini / Vertex AI**: AI copy generation (`lib/ai/gemini.ts`, dual backend).
  ...
  **`GEMINI_EMBEDDING_MODEL`** defaulting to `gemini-embedding-001` and a fixed
  ...
  key. It is unreachable unless **`MINK_AI_ENABLED=true`** and reads
